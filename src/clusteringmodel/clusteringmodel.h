...
class ClusteringModel : public InternalModel {
 ...
 private:
  void parse_comparison_measure(const XmlNode &node) {
    ...
    if (cm.exists_child("squaredEuclidean"))  { metric = Metric::SQUARED_EUCLIDEAN; return; }
    if (cm.exists_child("cityBlock"))         { metric = Metric::CITYBLOCK; return; }
    if (cm.exists_child("chebychev"))         { metric = Metric::CHEBYSHEV; return; }
  ...
  void parse_clustering_fields(const XmlNode &node, const std::shared_ptr<Indexer> &indexer) {
    for (const auto &cf : node.get_childs("ClusteringField")) {
      ...
      f.field_weight = cf.exists_attribute("fieldWeight")
                           ? to_double(cf.get_attribute("fieldWeight"))
                           : 1.0;
  ...
  void parse_clusters(const XmlNode &node) {
    ...
    for (const auto &cl : node.get_childs("Cluster")) {
      ...
      for (size_t i = 0; i < vals.size(); i++)
        c.centroid[static_cast<Eigen::Index>(i)] = to_double(vals[i]);
  ...
  static double field_compare(double xi, double ci, CompareFunc cf) {
    switch (cf) {
      case CompareFunc::ABS_DIFF:  return std::abs(xi - ci);
      case CompareFunc::GAUSS_SIM: return 1.0 - std::exp(-(xi - ci) * (xi - ci));
      case CompareFunc::DELTA:     return (xi != ci) ? 1.0 : 0.0;
      case CompareFunc::EQUAL:     return (xi == ci) ? 1.0 : 0.0;
    }
  ...
  double compute_distance(const Eigen::VectorXd &q, const Eigen::VectorXd &c) const {
    ...
    switch (metric) {
      ...
      case Metric::MINKOWSKI: {
        double sum = 0.0;
        for (size_t i = 0; i < n; i++)
          sum += fields[i].field_weight *
                 std::pow(field_compare(q[i], c[i], fields[i].compare_func), minkowski_p);
  ...
  std::string find_cluster(const Sample &sample) const {
    ...
    double best_val = is_similarity ? -std::numeric_limits<double>::max()
